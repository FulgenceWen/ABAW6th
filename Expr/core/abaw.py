...
class ABAW3Dataset(data.Dataset):
    def __init__(self, split='Train', transforms=None):
        ...
        if split != 'Test':
            ...
            data_dict = np.load(pathlib.Path(self.root_dir, '{}_test_set.npy'.format(self.task)).__str__(),
...
class ABAW3DataModule(LightningDataModule):
    def __init__(self):
        ...
        aug = [transforms.RandomHorizontalFlip(p=0.5),
               ...
               transforms.Resize(cfg.DATA_LOADER.IMG_SIZE),
               ]
        ...
        if cfg.DATA_LOADER.IMG_SIZE <= 224:
